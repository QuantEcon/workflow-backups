...
class TestBackupRepositories:
    ...
    @patch("src.backup.backup_manager.Github")
    def test_backup_no_matching_repos(
        self, mock_github, mock_s3_handler, mock_repo_matcher
    ):
...
class TestBackupSingleRepo:
    ...
    def test_backup_single_repo_success(
        self, mock_run, mock_github, 
        mock_s3_handler, mock_repo_matcher, mock_repos, tmp_path
    ):
    ...
    def test_backup_single_repo_clone_failure(
        self, mock_run, mock_github, 
        mock_s3_handler, mock_repo_matcher, mock_repos
    ):
    ...
    def test_report_with_backups(
        ...
        mock_s3_handler.list_backups.side_effect = [
            [  # lecture-python has 2 backups
                {"key": "backups/lecture-python/lecture-python-20251127.tar.gz", "size": 1024000, "last_modified": datetime(2025, 11, 27)},
                {"key": "backups/lecture-python/lecture-python-20251120.tar.gz", "size": 1020000, "last_modified": datetime(2025, 11, 20)},
            ],
            [  # lecture-julia has 1 backup
                {"key": "backups/lecture-julia/lecture-julia-20251127.tar.gz", "size": 512000, "last_modified": datetime(2025, 11, 27)},
            ],
        ]
